...
⁰ Fable 5는 비추론(none) 대신 **low** 실행: **332.5점** (v1=v2, $3.84 / 49K, 150/150 완주). 형사법 29번 1건이 v1 파싱 실패였으나 v2로 구제해도 오답이라 점수 동일.
...
1. **Fable 5는 모든 조건에서 1위.** max 362.5(96.7%), high로 낮춰도 355.0을 비용 1/5($15)로 달성. low까지 내리면 332.5를 **$3.84**에 — Opus 4.8의 비용대($3~6)에서 그보다 +17.5~35점 높다.
...
8. **전송 인프라가 상한을 만든다**: ChatGPT OAuth 스트림 ~15분 한도(Luna/Terra max), 128K 출력 한도(Sonnet max)가 각각 실측됨.
