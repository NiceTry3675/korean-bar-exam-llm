...
export default function CostScatterChart({
  ...
  // 만점이 주어지지 않으면 데이터 최고점을 상한으로 사용한다
  const yMax = maxScore > 0
    ? maxScore
    : Math.max(...validData.map(d => d.score), 1)
  // 최소값을 깔끔한 간격으로 계산
  const { min: yMin, interval: yInterval } = getNiceRange(dataMinScore, yMax)
