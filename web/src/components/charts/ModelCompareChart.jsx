...
export default function ModelCompareChart({
  ...
  const baseData = useMemo(() => {
    return dimensions.map(dimension => ({
      subject: dimension.nameKey ? t(dimension.nameKey) : dimension.label,
      _grid: 100
    }))
  }, [dimensions, t])
  ...
  const chartData = useMemo(() => {
    ...
    return sourceData.map((row, idx) => {
      const { key, maxScore } = dimensions[idx]
      const normalizedScore = modelScore.normalizedScores?.[key]
      const rawScore = normalizedScore ?? (
        maxScore > 0 ? ((modelScore[key] ?? 0) / maxScore) * 100 : 0
      )
      return {
        ...row,
        [hoveredModel]: rawScore
      }
    })
  }, [data, baseData, hoveredModel, selectedModels, allScores, dimensions])
