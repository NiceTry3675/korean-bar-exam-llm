...
import { getBenchmarkMode, getLocalizedRegistryText } from '@/utils/benchmarkRegistry'

/**
 * @brief 헤더 컴포넌트
 * @param {Object} props - 헤더 제어 및 현재 벤치마크 설정
 */
export default function Header({
  ...
  // 모드가 하나뿐인 벤치마크에서는 토글이 같은 화면으로만 이동하므로 감춘다
  const hasMultipleModes = Object.keys(benchmark?.modes || {}).length > 1
  const [showTooltip, setShowTooltip] = useState(false)
  const tooltipTimeout = useRef(null)
  const language = i18n.language
  const benchmarkTitle = getLocalizedRegistryText(benchmark?.title, language) || t('header.title')
  const benchmarkTitleShort = getLocalizedRegistryText(benchmark?.titleShort, language) || benchmarkTitle
  const benchmarkSubtitle = getLocalizedRegistryText(benchmark?.description, language)
    || t('header.subtitle')
  const modeConfig = getBenchmarkMode(benchmark, mode)
  const modeLabel = getLocalizedRegistryText(modeConfig?.label, language)
    || (isHardMode ? t('header.modeHard') : t('header.modeDefault'))
  const modeDescription = getLocalizedRegistryText(modeConfig?.description, language)
        ...
        <div className="flex-1 md:flex-none flex items-center gap-3 md:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-lg md:text-2xl font-bold truncate">
              <span className="md:hidden">{benchmarkTitleShort}</span>
              <span className="hidden md:inline">{benchmarkTitle}</span>
            </h1>
          ...
          {benchmarks.length > 1 && (
            <select
              ...
              className="hidden lg:block bg-gray-800 border border-white/20 rounded-lg px-2 py-2 text-sm text-gray-100 max-w-52"
              aria-label={t('header.selectBenchmark')}
            >
              {benchmarks.map(item => (
                <option key={item.id} value={item.id}>
                  {getLocalizedRegistryText(item.title, language) || item.id}
                </option>
              ))}
            </select>
          )}
