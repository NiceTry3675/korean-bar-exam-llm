...
![모델별 점수표](docs/images/점수표.png)

**토큰 사용량**

![총 토큰 사용량](docs/images/토큰_사용량.png)

> 점수표 오른쪽 끝의 **145/150**은 맞힌 문항 수입니다. 답을 추출하지 못한 문항은
> 0점 처리되며, 원인은 [응답을 얻지 못한 경우](#응답을-얻지-못한-경우)에 정리했습니다.
>
> 점수표의 **v2** 값은 형식을 어긴 응답까지 구제해 채점한 병행 표기입니다. 공식 점수는
> v1 기준이며, 자세한 내용은
> [공식 점수(v1)와 병행 표기(v2)](#공식-점수v1와-병행-표기v2)에 있습니다.
>
...
과목별 점수 그래프, 문항별 정오표, 모델별 비용표는 이 문서에 싣지 않았습니다.
[인터랙티브 대시보드](https://nicetry3675.github.io/korean-bar-exam-llm/)에서 확인하실 수
있고, 같은 수치가 워크북 `제15회 변호사시험 LLM 풀이.xlsx`에도 들어 있습니다.
...
(Luna none 5건)는 성격이 다릅니다. 다섯 건 모두 **"선택지에 정답이 없다"며 번호 고르기를
거부**하고 `정답: 없음`·`해당 없음`·`N`으로 끝냈습니다. Google 계열은 형식 위반 자체가 0건이었습니다.
